...
function mostrarErroresCampos(errores) {
  const el = document.getElementById('co-field-errors')
  if (!el) return
  if (!errores.length) { el.style.display = 'none'; el.innerHTML = ''; return }
  el.style.display = 'block'
  el.innerHTML = '⚠️ Completa los siguientes campos:<br>• ' + errores.join('<br>• ')
  el.scrollIntoView({ behavior: 'smooth', block: 'nearest' })
}

// ── Simulación de cobro con tarjeta ─────────────────────
function simularCobroTarjeta(num, total) {
  return new Promise(resolve => {
    // Simula procesamiento 1.5s → siempre aprueba (integrar pasarela real aquí)
    setTimeout(() => resolve({ aprobado: true, autorizacion: 'AUTH-' + Math.random().toString(36).slice(2,8).toUpperCase() }), 1500)
  })
}

// ── Construcción del mensaje factura para WhatsApp ──────
function buildFacturaWA(num, nombre, tel, ciudad, notas, pago, carritoSnap, total) {
  const fecha = new Date().toLocaleDateString('es-HN', { day:'2-digit', month:'long', year:'numeric' })
  const items = carritoSnap.map(i =>
    `  • ${i.nombre}${i.varTxt ? ` (${i.varTxt})` : ''} × ${i.qty} = ${L(i.qty * i.precio)}`
  ).join('\n')

  const instrucciones = {
    'Efectivo al recibir': '💵 *Pago al recibir:* Te cobraremos en efectivo al momento de la entrega.',
    'Transferencia BAC':   '🏦 *Transferencia BAC:*\nCuenta: [Tu número de cuenta]\nTitular: [Tu nombre]\nEnvíanos el comprobante por WhatsApp.',
    'PayPal':              '💻 *PayPal:* Realiza el pago a [tu-correo@paypal.com] y envíanos la captura de pantalla.',
  }

  return `🌸 *MARY KAY HONDURAS*\n` +
    `Consultora oficial · Catálogo Primavera 2026\n` +
    `━━━━━━━━━━━━━━━━━━━━━\n` +
    `📋 *PEDIDO ${num}*\n` +
    `📅 Fecha: ${fecha}\n` +
    `━━━━━━━━━━━━━━━━━━━━━\n` +
...
async function hacerPedido() {
  ...
  const tel    = g('co-tel')
  const ciudad = g('co-ciudad')
  const pago   = g('co-pago')
  const notas  = g('co-notas')
  const promo  = document.getElementById('co-promo')?.checked ?? true
  const registrar = document.getElementById('co-registrar')?.checked

  // ── Validación de campos obligatorios ──
  const errores = []
  if (!nombre)  errores.push('Nombre completo')
  if (!tel)     errores.push('Número de WhatsApp')
  if (!ciudad)  errores.push('Ciudad / Departamento')
  if (!notas)   errores.push('Dirección de entrega')

  if (pago === 'Tarjeta') {
    if (!g('card-num')) errores.push('Número de tarjeta')
    if (!g('card-exp')) errores.push('Fecha de vencimiento')
    if (!g('card-cvv')) errores.push('CVV')
  }
  ...
  btn.disabled = true
  btn.textContent = pago === 'Tarjeta' ? '🔒 Procesando pago...' : 'Procesando...'
  ...
  try {
    ...
    if (userActual && clienteAct) {
      ...
      await db.from('clientes').update({
        nombre, telefono: tel, ciudad: ciudad || null,
        acepta_promociones: promo
      }).eq('id', clienteId)
    } else {
      const { data: cli } = await db.from('clientes').select('id').eq('telefono', tel).maybeSingle()
      if (cli) {
        clienteId = cli.id
        await db.from('clientes').update({ nombre, ciudad: ciudad || null, acepta_promociones: promo }).eq('id', clienteId)
      } else {
        const { data: nvo } = await db.from('clientes').insert({
          nombre, telefono: tel, ciudad: ciudad || null,
          etiqueta: 'Nueva', acepta_promociones: promo
        }).select('id').single()
        clienteId = nvo?.id
      }
      if (registrar) {
    ...
    if (pago === 'Tarjeta') {
      ...
      const resultado = await simularCobroTarjeta(nombre, total)
      if (!resultado.aprobado) {
        btn.disabled = false
        btn.textContent = '💳 Pagar y confirmar'
        mostrarErroresCampos(['El pago con tarjeta fue rechazado. Verifica los datos e intenta de nuevo.'])
        return
      }
      estadoPedido = 'Pagado'
      authCode = resultado.autorizacion
    }
    ...
    const facturaEl = document.getElementById('confirm-factura')

    if (pago === 'Tarjeta') {
      // Pago con tarjeta — mostrar aprobación
      waBtn.style.display    = 'none'
      facturaEl.style.display = 'none'
      cardOk.style.display   = 'flex'
      cardOk.innerHTML = `💳 Pago aprobado${authCode ? ` · Auth: <strong>${authCode}</strong>` : ''}`
    } else {
      // Otros métodos — generar factura WA
      const facturaTexto = buildFacturaWA(num, nombre, tel, ciudad, notas, pago, carritoSnap, total)

      // Construir número limpio para WA (quitar guiones/espacios)
      const telLimpio = tel.replace(/\D/g,'')
      const waNum = telLimpio.startsWith('504') ? telLimpio : '504' + telLimpio

      // Enlace WA al número de la CLIENTA (desde el teléfono de la tienda)
      waBtn.href = `https://wa.me/${waNum}?text=${encodeURIComponent(facturaTexto)}`
      waBtn.style.display    = 'flex'
      cardOk.style.display   = 'none'

      // Mostrar resumen de la factura en el modal
      const metodosInfo = {
        'Efectivo al recibir': '💵 Paga en efectivo al recibir tu pedido.',
        'Transferencia BAC':   '🏦 Realiza la transferencia BAC y envía el comprobante.',
        'PayPal':              '💻 Realiza el pago por PayPal y envía la captura.',
      }
      facturaEl.style.display = 'block'
      facturaEl.innerHTML = `
        <div style="font-weight:700;color:#7c3aed;margin-bottom:6px">📋 Tu factura — ${num}</div>
        ${carritoSnap.map(i=>`<div style="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px dashed #e9d5ff">
          <span>${i.nombre}${i.varTxt?` <small>(${i.varTxt})</small>`:''} × ${i.qty}</span>
          <strong>${L(i.qty*i.precio)}</strong>
        </div>`).join('')}
        <div style="display:flex;justify-content:space-between;margin-top:8px;font-size:15px;font-weight:800;color:#1f2937">
          <span>Total</span><span>${L(total)}</span>
        </div>
        <div style="margin-top:10px;padding:8px;background:#ede9fe;border-radius:6px;color:#5b21b6;font-size:12px">
          ${metodosInfo[pago] || ''}
        </div>`
    }
    ...
    mostrarErroresCampos(['Error al procesar el pedido. Por favor intenta de nuevo.'])
  }
  btn.disabled = false
